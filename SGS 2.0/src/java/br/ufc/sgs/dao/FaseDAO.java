...
public class FaseDAO extends ExecutaSQL{
    ...
    public Collection<Fase> getFasesDoConcurso(int id_concurso) {
       try {
            String sql = "SELECT id, titulo, descricao, dataInicio, dataTermino, criterioDeSelecao FROM FASE WHERE id_concurso = ?";
            PreparedStatement ps = getConexao().prepareStatement(sql);
            ...
            while(rs.next()){
                ...
                Collection<String> documentosRequisitados = new DocumentosRequisitadosDAO(getConexao()).getDocumentosRequisitados(id_concurso);
                fases.add(new Fase(id_fase, titulo, descricao, dataInicio, dataTermino, null, documentosRequisitados, participantes, avaliadores));
            }
            return fases;
